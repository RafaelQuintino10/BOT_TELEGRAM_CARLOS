...
# FUNÇÃO PARA MONITORAR OS GRUPOS OU CHATS PRIVADOS.
async def monitorar_grupo(chat_id, chat_nome, context):
    try:
        ...
        while True:
            try:
                # SELECIONA AS MENSAGENS DA TABELA PARA O CHAT ID ATUAL E QUE ESTIVEREM COM 'ENVIADA' IGUAL A 0.
                cursor.execute('SELECT * FROM TLG_NOTIFICACOES WHERE ENVIADA = 0 AND COD_GRUPO = %s ORDER BY DTNOTIFIC ASC', (chat_id,))
                dados = cursor.fetchall()
                if dados:
                    for linha in dados:
                        # DESESTRUTURAÇÃO SEM ÍNDICES. 
                        COD_GRUPO, EMPR, CODFUN, DTNOTIFIC, DSNOTIFIC, ENVIADA, TIMELOG, NOMEGRUPO = linha
                        ...
                        cursor.execute('UPDATE TLG_NOTIFICACOES SET ENVIADA = 1, TIMELOG =  NOW()::timestamp(0) WHERE DSNOTIFIC = %s AND COD_GRUPO = %s', (DSNOTIFIC, chat_id))
                        conexao.commit()
                else:
